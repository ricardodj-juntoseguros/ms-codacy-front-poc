...
open_kong(){
    ...
    do
        PLUGIN=$(kubectl get kongplugin -n "${NAMESPACE_1}" "kong-ip-restriction-${SERVICE}" 2>&1 || EXIT_CODE=$?)

        if [[ ! "$PLUGIN" =~ "NotFound"  ]]; then
            echo "Adding CircleCI IP Address to Whitelist on Kong"
            kubectl patch kongplugin -n "${NAMESPACE_1}" "kong-ip-restriction-${SERVICE}" --type=json -p '[{"op": "add", "path": "/config/allow/-", "value": "'${CIRCLECI_IP}'"}]'
            echo "Adding Cloudflare IPs Address to Whitelist on Kong"
            for SCLOUDFLARE_POINT in ${CLOUDFLARE_IPS}
            do
                kubectl patch kongplugin -n "${NAMESPACE_1}" "kong-ip-restriction-${SERVICE}" --type=json -p '[{"op": "add", "path": "/config/allow/-", "value": "'${SCLOUDFLARE_POINT}'"}]'
            done
        else
    ...
    for SERVICE in ${SERVICE_LIST}
    do
        PLUGIN=$(kubectl get kongplugin -n "${NAMESPACE_2}" "kong-ip-restriction-${SERVICE}" 2>&1 || EXIT_CODE=$?)

        if [[ ! "$PLUGIN" =~ "NotFound"  ]]; then
            echo "Adding CircleCI IP Address to Whitelist on Kong"
            kubectl patch kongplugin -n "${NAMESPACE_2}" "kong-ip-restriction-${SERVICE}" --type=json -p '[{"op": "add", "path": "/config/allow/-", "value": "'${CIRCLECI_IP}'"}]'
            echo "Adding Cloudflare IPs Address to Whitelist on Kong"
            for SCLOUDFLARE_POINT in ${CLOUDFLARE_IPS}
            do
                kubectl patch kongplugin -n "${NAMESPACE_2}" "kong-ip-restriction-${SERVICE}" --type=json -p '[{"op": "add", "path": "/config/allow/-", "value": "'${SCLOUDFLARE_POINT}'"}]'
            done
        else
            echo "Adding CircleCI IP Address to Whitelist on k8s"
            # WHITE_LIST="${INGRESS_CIDR_BLOCKS},${CICLECI_IP_ADDRESS}"
...
close_kong(){
    ...
    do
        ...
        if [[ ! "$PLUGIN" =~ "NotFound"  ]]; then
            echo "Recovering default whitelist on kong"
            WHITE_LIST=$(kubectl get kongplugin -o json -n "${NAMESPACE_1}" "kong-ip-restriction-${SERVICE}" | jq .config.allow | jq 'del(.[] | select(. == "'${CIRCLECI_IP}'"))' | jq -c )
            kubectl patch kongplugin -n "${NAMESPACE_1}" "kong-ip-restriction-${SERVICE}" --type=json -p '[{"op": "replace", "path": "/config/allow", "value":'${WHITE_LIST}'}]'

            for SCLOUDFLARE_POINT in ${CLOUDFLARE_IPS}
            do
                WHITE_LIST=$(kubectl get kongplugin -o json -n "${NAMESPACE_1}" "kong-ip-restriction-${SERVICE}" | jq .config.allow | jq 'del(.[] | select(. == "'${SCLOUDFLARE_POINT}'"))' | jq -c )
                kubectl patch kongplugin -n "${NAMESPACE_1}" "kong-ip-restriction-${SERVICE}" --type=json -p '[{"op": "replace", "path": "/config/allow", "value":'${WHITE_LIST}'}]'
            done
    ...
    do
        ...
        if [[ ! "$PLUGIN" =~ "NotFound"  ]]; then
            echo "Recovering default whitelist on kong"
            WHITE_LIST=$(kubectl get kongplugin -o json -n "${NAMESPACE_2}" "kong-ip-restriction-${SERVICE}" | jq .config.allow | jq 'del(.[] | select(. == "'${CIRCLECI_IP}'"))' | jq -c )
            kubectl patch kongplugin -n "${NAMESPACE_2}" "kong-ip-restriction-${SERVICE}" --type=json -p '[{"op": "replace", "path": "/config/allow", "value":'${WHITE_LIST}'}]'

            for SCLOUDFLARE_POINT in ${CLOUDFLARE_IPS}
            do
                WHITE_LIST=$(kubectl get kongplugin -o json -n "${NAMESPACE_2}" "kong-ip-restriction-${SERVICE}" | jq .config.allow | jq 'del(.[] | select(. == "'${SCLOUDFLARE_POINT}'"))' | jq -c )
                kubectl patch kongplugin -n "${NAMESPACE_2}" "kong-ip-restriction-${SERVICE}" --type=json -p '[{"op": "replace", "path": "/config/allow", "value":'${WHITE_LIST}'}]'
            done
