...
function RequestForgotPassword() {
  ...
  const handleSubmit = async () => {
    if (emailValidator(email)) {
      ...
      await AuthApi.forgotPassword(email)
        .then(() => navigate('/request-forgot-password-finish'))
        .catch(() => {
  ...
  return (
    <div className={styles['request_forgot_password_container__wrapper']}>
      <LogoVendors />
      <h1 className={styles['request_forgot_password_title']}>
        Confirme seu e-mail para recuperar seu acesso.
      </h1>
      <div className={styles['request_forgot_password__form']}>
        <InputBase
        ...
        <div>
          <Button
            ...
            fullWidth
          >
            {isSubmitting ? ((<LoadingSpinner />) as any) : 'Recuperar acesso'}
          </Button>
        </div>
