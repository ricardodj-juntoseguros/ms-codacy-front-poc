...
import { Button,InputBase,LinkButton,Checkbox,Modal,makeToast } from "junto-design-system";
...
export function FirstAccessForm({
  ...
  const validatePasswold = async () => {
    const result = await validate(PasswordSchema, {email:userEmail, password, confirmPassword});
  ...
  const handleLogin = async () => {
    setIsSubmitting(true);
     await AuthApi.postAuth(userEmail,password)
      .then(response => {AuthApi.setUserAccessCookie(response)})
      .catch(() =>
        history.push('/')
        )
      setIsSubmitting(false);
  ...
  return (
    <div className={styles['first_access_wrapper']}>
        <h1 className={styles['first_access_title']}>Para sua segurança, precisamos que você defina uma senha.</h1>
        <div
        className={styles['first_access_form']}
        >
        <InputBase
        ...
        { showValidation &&
          <div className={styles['first_access_validation_password']}>
            <div className={styles['first_access_validation_password_rule']}>
              <div>{renderIconCheckPassword(VALIDATION_MESSAGES.minChar)}<span className={styles['first_access_validation_password_rule_text']}> Mínimo de 10 caracteres </span></div>
              <div>{renderIconCheckPassword(VALIDATION_MESSAGES.differentEmail)}<span className={styles['first_access_validation_password_rule_text']}> Diferente do e-mail </span></div>
              <div>{renderIconCheckPassword(VALIDATION_MESSAGES.notNumberSequence)}<span className={styles['first_access_validation_password_rule_text']}> Sem sequência (123456...) </span></div>
              <div>{renderIconCheckPassword(VALIDATION_MESSAGES.specialChar)}<span className={styles['first_access_validation_password_rule_text']}> Um caractere especial (&*ˆ$...) </span></div>
              <div>{renderIconCheckPassword(VALIDATION_MESSAGES.samePasswords)}<span className={styles['first_access_validation_password_rule_text']}> As duas senhas precisam ser iguais </span></div>
              <div>{renderIconCheckPassword(VALIDATION_MESSAGES.min3charDifferent)}<span className={styles['first_access_validation_password_rule_text']}> 3 tipos de caracteres diferentes (maiúsculas, minúsculas e números) </span></div>
             </div>
          </div>
        }
         { showTerms &&
