...
export const useIssuance = () => {
  ...
  const { typeOfAuthorization, approvalContacts } = useSelector(selectCommercialAuthorization);
  const { advanceStep } = useFlow();
  const { setIssuedAt, setProtocols } = proposalActions;

  const onNext = useCallback((stepName: string, issuedAt: string | null, protocols: SubmitToApprovalOrIssuanceResultDTO['protocols']) => {
  ...
  const submitToApproval = useCallback((stepName: string) => {
    if (!currentQuote) return;
    setLoadingIssuance(true);
    const payload = issuanceAdapter(comments, [], approvalContacts, typeOfAuthorization);
    ProposalApi.submitToApproval(currentQuote.identification.NewQuoterId, payload)
      .then(response => {
  ...
  }, [approvalContacts, comments, currentQuote, history, onNext, typeOfAuthorization]);

  const postIssuance = useCallback((stepName: string) => {
    if (!currentQuote) return;
    setLoadingIssuance(true);
    const payload = issuanceAdapter(comments, [], approvalContacts, typeOfAuthorization);
    IssuanceApi.postIssuance(currentQuote.identification.NewQuoterId, payload)
      ...
      .catch(error => makeToast('error', handleError(error)))
      .finally(() => setLoadingIssuance(false));
  }, [currentQuote, comments, approvalContacts, typeOfAuthorization, onNext, history]);

  const createIssuanceOrSubmitToApproval = useCallback(async (stepName: string) => {
    const userProfile = BrokerPlatformAuthService.getUserProfile();
    if (isAutomaticPolicy && userProfile === ProfileEnum.COMMERCIAL && typeOfAuthorization === 'sendToApproval') {
      submitToApproval(stepName);
      return;
    }
