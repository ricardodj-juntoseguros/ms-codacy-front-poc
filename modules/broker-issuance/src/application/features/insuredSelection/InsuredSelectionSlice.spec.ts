...
import { insuredSelectionActions, searchInsured } from "./InsuredSelectionSlice";
...
describe('InsuredSelectionSlice', () => {
  ...
  it('should be able to correctly update the addresses', async () => {
    await store.dispatch(insuredSelectionActions.setInsuredAddressesOptions([insuredMock.addresses[0], insuredMock.addresses[1]]));
    const { insuredSelection } = store.getState();
