...
describe('ModalitySelectionApi', () => {
  ...
  it('should not broke retreveing the modalities', () => {
    ...
    const response = ModalitySelectionApi.fetchModalities(
      brokerFederalId,
  ...
  it('fetchModalities should call bff service correctly', async () => {
    const policyholderFederalId = '99999999999999';
    const brokerFederalId = '8888888888888';
    const mockGet = jest
    ...
    expect(result).toStrictEqual([modalityDefaultMock]);
  });
});
