...
const PolicyholderContact: React.FunctionComponent<GenericComponentProps> = ({ handleNextStep, updateTitle }) => {
  const { policyholderContact } = useSelector(selectProposal);
  const [disabledFields, setDisabledFields] = useState(!!policyholderContact.id);
  ...
  useEffect(() => {
    const getContacts = () => {
      const query = window.location.search ? window.location.search.substring(11, window.location.search.length) : '91833813000118'
      PolicyholderContactAPI.getContacts(query).then(response => {
        if(response.length >= 1) {
          dispatch(proposalActions.setPolicyholderContact(response[0]));
          setDisabledFields(true);
          updateTitle && updateTitle('Este será o %STRONG% Aquele que receberá a solicitação para aprovação.', ['contato da empresa contratada.'])
        }
      }).catch(error => {
        makeToast('warning', error.data.data.message)
      });
    }
  ...
  const handleContactName = (name: string) => {
    dispatch(proposalActions.setPolicyholderContact({...policyholderContact, name}));
  }

  const handleContactEmail = (email: string) => {
    dispatch(proposalActions.setPolicyholderContact({...policyholderContact, email}));
  }

  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault();

    const result = await validate(PolicyholderContactSchema, policyholderContact);
    if(!result.isValidForm) {
      setErrors(result.errors);
      return;
    }

    handleNextStep(`${policyholderContact.name} - ${policyholderContact.email}`);
  }
  ...
  return (
    ...
    >
      ...
      />
      <Button data-testid="policyholderContact-button-next" type="submit" fullWidth disabled={disabledButton}>
        Avançar
      </Button>
    </form>
  );
