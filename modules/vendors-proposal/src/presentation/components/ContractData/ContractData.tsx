...
import { fetchProjects, projectSelectionActions, selectProjectSelection } from "../../../application/features/projectSelection/ProjectSelectionSlice";
...
const ContractData: React.FunctionComponent<GenericComponentProps> = ({ handleNextStep }) => {
  ...
  const { contractNumber, contractValue, hasProject, project } = useSelector(selectProposal);
  const { projectSearchValue, projectOptionsMapped, } = useSelector(selectProjectSelection);
  ...
  const handleProject = useCallback((project: SearchOptions | null) => {
  ...
  const handleSearchProjects = useCallback((value: string) => {
