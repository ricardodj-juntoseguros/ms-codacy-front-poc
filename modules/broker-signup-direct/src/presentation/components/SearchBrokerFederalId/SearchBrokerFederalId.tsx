...
export function SearchBrokerFederalId({handleGoNextClick}: SearchBrokerFederalIdProps) {
  ...
  const getShowAlertErrorSusep = () => {
    setAlertText('Por favor entre em contato com a regional mais próxima de você.')
    setShowAlert(true);
  ...
  const getShowAlert = () => {
    ...
    if(!hasSusepError && statusBrokerRegistry?.status !== undefined){
      ...
      if(statusSusep?.retorno?.situacao.toUpperCase() !== 'ATIVO' && statusBrokerRegistry?.status  !== RegisterBrokerTypeEnum.INVALID){
        setAlertText('Esse CNPJ não é de uma corretora de seguros. Informe o CNPJ de uma corretora para continuar.')
        setShowAlert(true);
        setIsDisableButtonSignup(true);
        return;
      }
    ...
    <div className={styles['search-broker-federalId-wrapper']}>
      ...
      { statusBrokerRegistry?.information?.brokerCompanyName && !showAlert &&
        <div className={styles['broker-information_wrapper']}  data-testid="alert-broker-regisry">
            <span className={styles['broker-information_label']}>Corretora encontrada</span>
