...
import { HAVE_PROBLEM } from '../../../constants/problemSendCodeValidaitonEmail'
...
const ValidationBrokerEmailContainer = ({ history }: RouteComponentProps) => {
  ...
  const onSubmit = () => {
    ...
    setSignupIsDirect(broker.hasProductDamageInsurance && broker.renewRegistration && broker.susepSituation);
  ...
  const compareEmailBrokerToEmailResponsible = () =>{
    if(broker.information.email && responsibleInformation.emailBroker && broker.information.email === responsibleInformation.emailBroker) return true;

    const brokerEmailDomain = broker.information.email.trim().split('@');
    const responsibleEmailDomain = responsibleInformation.emailBroker.trim().split('@');

    if(brokerEmailDomain.length < 1 || responsibleEmailDomain.length < 1) return false;

    if(brokerEmailDomain[1] === responsibleEmailDomain[1] && !EMAIL_PROVIDER_LIST.includes(responsibleEmailDomain[1].toUpperCase())) return true;
  ...
  const templateModal = {
    title: { value: HAVE_PROBLEM.title},
    text:  { value: HAVE_PROBLEM.text},
  }

  return (
    <div className={styles['validation_broker_email_container_wrapper']}>
    <div className={styles['validation_broker_email_container_section_form_logo']}>
     <LogoJunto />
     </div>
 <div className={styles['validation_broker_email_container_section_form']}>
     <div className={styles['validation_broker_email_container_section_form_section']}>
       <h1>Enviamos um código para o seu e-mail</h1>
       <h2>Informe o código de 4 dígitos enviado para o e-mail <span>{responsibleInformation.emailBroker}</span></h2>
       <ValidationEmailCode userPath={broker.pathUpdate} codeIsValid={codeIsValid} onSetCodeIsValid={setCodeIsValid} codeIsComplet={codeIsComplet} onSetCodeIsComplet={setCodeIsComplet}/>
       <div className={styles['validation_broker_email_container_section_form_go_next']}>
          <Button
              data-testid="button-responsible-information-registry"
              size={codeIsValid && codeIsComplet ? "large" : "medium"}
              onClick={codeIsValid && codeIsComplet ? onSubmit : onRetrySendEmail}
              variant={codeIsValid && codeIsComplet ? "primary" : "secondary"}
              disabled={buttonDisabled}>
                     {codeIsValid && codeIsComplet ? "Continuar" : "Enviar novamente"}
          </Button>
          <div className={styles['validation_broker_email_container_section_form_go_next_link_butoon']}>
         { !codeIsValid &&
         <LinkButton onClick={() => setIsOpen(true)} label="Tive um problema"/>
        }
         <Modal
                size="large"
                open={isOpen}
